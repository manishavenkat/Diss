...
def stratified_train_test_split(metadata_file, fraction=1.0, test_size=0.20, random_state=42):
    ...
    split_file = 'exp10_sl_train_test.json'
    ...
    if os.path.exists(split_file):
        with open(split_file, 'r') as f:
            ...
            with open(metadata_file, 'r') as f:
                ...
                category_mapping = {int(k): v for k, v in split_data['category_mapping'].items()}
                reverse_category_mapping = {int(k): v for k, v in split_data['reverse_category_mapping'].items()}
            return (split_data['train_indices'], split_data['test_indices'], 
                category_mapping, reverse_category_mapping, filtered_metadata)
    else:
        # Load metadata
        with open(metadata_file, 'r') as f:
            reader = csv.DictReader(f)
...
model.load_state_dict(torch.load('SL/exp10_sl.pt', map_location=device))
...
## Uncomment to plot all in(accurate) preds
...
# Create the plot with only correct predictions
fig = px.scatter(df_correct, x='x', y='y', color='category', 
                 symbol='predicted_category',
                 hover_data=['category', 'predicted_category', 'title', 'segment_id'], 
                 color_discrete_map=color_map, 
